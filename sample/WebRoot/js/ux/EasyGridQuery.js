/**
 * @class Ext.ux.EasyGridQuery
 * @extends Ext.grid.GridPanel
 */

Ext.define('Ext.ux.EasyGridQuery', {
			extend : 'Ext.grid.Panel',
			alias : ['widget.easygridquery'],
			...
			initComponent : function() {
				...
				if(this.needCheckboxColumn){
					this.selModel = Ext.create('Ext.selection.CheckboxModel',{injectCheckbox : this.checkboxPos || 'last'});
				}
			...
			createColumns : function() {
				var cols = [new Ext.grid.RowNumberer({width:18,align : 'center'})];
			...
			createRecord : function() {
                ...
                if (Ext.isFunction(this.viewPanel.setIsView))
					this.viewPanel.setIsView(false);
		        ...
		        if (r != false) {
		            Ext.Msg.confirm('提示', '确定删除吗?', function(btn) {
		                        if (btn == 'yes') {//只有保存后的数据才去调用
		                            if (!Ext.isEmpty(r.get(this.recordId || 'id'))) {
		                                if (this.parentPanel.removeRecord(this.store, r,this.gridName)) {// 如果能正确删除
			...
			createViewPanel : function() {
				...
				this.viewPanel = new viewPanleClass({
							...
							parentGrid : this,
							windowWidth : this.subWindowWidth
						});
			},
